...
class MultiLineTextInput(TextInput):
    ...
    def _set_view(self):
        ...
        self.view = View(0, self.max_lines * (self.width))
    ...
    def _wrap_text(self, text: str) -> str:
        ...
        return "\n".join(
            [text[i : i + self.width] for i in range(0, len(text), self.width)]
        )
    ...
    def update_view(self, prev: int, curr: int) -> None:
        ...
        if prev >= self.view.start and curr < self.view.start:
            self.view.shift_left(self.width)

        elif prev <= self.view.end and curr >= self.view.end:
            self.view.shift_right(self.width, max_val=ceil(len(self.value), self.width))
    ...
    async def handle_keypress(self, key: str) -> None:
        ...
        match key:
            ...
            # EXTRAS
            case "home":
                self._cursor_position = floor(self._cursor_position, self.width)
            ...
            case "end":
                self._cursor_position = min(
                    ceil(self._cursor_position + 1, self.width),
                    len(self.value),
                )
            ...
            case "up":
                self._cursor_position = max(0, self._cursor_position - self.width)

            case "down":
                self._cursor_position = min(
                    self._cursor_position + self.width, len(self.value)
                )
    ...
    def render_panel(self, text: TextType) -> RenderableType:
        ...
        if not self.fixed:
            height = 1 + ((len(self.value) - self.view.start) or 1) // self.width
